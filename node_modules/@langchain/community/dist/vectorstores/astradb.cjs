...
class AstraDBVectorStore extends vectorstores_1.VectorStore {
    ...
    async similaritySearchVectorWithScore(query, k, filter) {
        ...
        await cursor.forEach(async (row) => {
            const { $similarity: similarity, $vector: _vector, [this.idKey]: _id, [this.contentKey]: content, ...metadata } = row;
    ...
    async maxMarginalRelevanceSearch(query, options) {
        ...
        topMmrMatches.forEach((match) => {
            const { $similarity: _similarity, $vector: _vector, [this.idKey]: _id, [this.contentKey]: content, ...metadata } = match;
            const doc = {
                pageContent: content,
                metadata,
            };
